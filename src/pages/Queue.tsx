...
import { getQueuedDocuments, getProcessedDocuments, getFailedDocuments, deleteDocument, setDocumentPriority } from "../lib/api/Api";
...
const Queue = () => {
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      addToast({ type: 'error', message: 'Failed to fetch documents.' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSetPriority = (id: string) => {
    ...
    setConfirmationModal({
      ...
      onConfirm: async () => {
        try {
          await setDocumentPriority(id, !doc.isPriority, addToast);
  ...
  const handleDelete = (id: string, tab: 'Queued' | 'Failed') => {
    ...
    setConfirmationModal({
        ...
        onConfirm: async () => {
            try {
                await deleteDocument(id, addToast);
                await fetchDocuments();
                        ...
                        {activeTab === "Queued" &&
                          user?.role === "admin" && (
                            <>
                              ...
                              <button
                                onClick={() =>
                                  handleDelete(selectedDocument.id, "Queued")
                                }
                                disabled={
                                  selectedDocument.status === "Processing"
                                }
                          ...
                          <>
                            ...
                            {user?.role === "admin" && (
                                <>
                                    ...
                                    <button
                                        onClick={() =>
                                            handleDelete(selectedDocument.id, "Failed")
                                        }
                                        className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-md font-semibold shadow-sm transition-all border ${
                                            theme === "dark"
                                                ? "bg-red-900/40 border-red-700/60 text-red-300 hover:bg-red-900/60"
