...
import { Plus, Banknote, FilePieChart, FileDiff, AlertTriangle, TrendingUp, Wallet } from 'lucide-react';
    ...
    if (user?.role !== 'admin') {
        return (
            <div className="flex flex-col items-center justify-center h-full text-center p-8">
                ...
                <button onClick={() => navigate('/queue')} className="mt-6 bg-violet-600 text-white font-bold py-2 px-4 rounded-lg">Go to Queue</button>
            </div>
        )
    }
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 animate-fade-in-up">
            <div className="lg:col-span-3 space-y-6">
                ...
                <DashboardStatusTable />
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     ...
                     <ChartCard title="Invoice Volume" icon={FilePieChart}>
                          <ResponsiveContainer width="100%" height="100%">
            ...
            <div className="lg:col-span-1 space-y-6">
                <ProcessStepper />
                <div className={`${cardClasses} flex flex-col items-center justify-center text-center`}>
                   <div className="flex items-center justify-center w-16 h-16 rounded-full bg-red-100 dark:bg-red-900/50 mb-4">
                       <FileDiff className="w-8 h-8 text-red-600 dark:text-red-400"/>
                   </div>
                   <p className={`text-5xl font-bold ${textHeader}`}>14</p>
                   <h3 className={`font-semibold mt-1 ${textPrimary}`}>Invoice Exceptions</h3>
                   <p className="text-sm text-gray-500 dark:text-gray-400">Needs manual intervention</p>
                </div>
