...
const Upload = () => { 
    ...
    // NEW: State to track if an upload is in progress
    const [isUploading, setIsUploading] = useState<boolean>(false);
    // NEW: State to store the progress percentage of each file
    ...
    const handleProceed = async (): Promise<void> => { 
        ...
        // This callback now needs to be adapted for a single progress value
        // The API function gives one percentage for the whole batch
        const handleUploadProgress = (progress: number) => {
            // Update all files in the batch with the same overall progress
        ...
        try { 
            // CORRECTED LINE: Pass all three required arguments
            const response = await api.uploadFiles(filesToUpload, handleUploadProgress, addToast); 
            ...
            if (response && response.success) { 
                ...
                // Error toast is now handled by the API function's catch block,
                // so you could remove this else block if you want to avoid double toasts.
            } 
        } catch (error) { 
            // Error is caught and handled inside api.ts, but component execution also stops.
                ...
                <div className={`lg:col-span-2 flex flex-col rounded-lg border min-h-0 ${theme === 'dark' ? 'bg-gray-900/50 border-gray-800' : 'bg-white border-gray-200'}`}> 
                        ...
                        <>
                            ...
                            <div className="overflow-y-auto p-4 space-y-4 flex-grow">
                                {files.slice(0, MAX_FILES).map((file) => (
                                    <div key={file.name} className="animate-fade-in-up">
                                        <div className="flex justify-between items-center mb-1.5">
                                            <p className={`text-sm font-medium truncate pr-4 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`} title={file.name}>
                                                {file.name}
                                            </p>
                    ...
                    <div className="mt-auto p-4 border-t border-inherit flex-shrink-0"> 
                        <button  
                            ...
                            // NEW: Also disable the button while uploading
                        ...
                        > 
                            {/* NEW: Change button text during upload */}
                            {isUploading ? 'Uploading...' : 'Upload and Proceed'} 
                        </button> 
                    </div> 
                </div> 
            </main> 
